...
def modbus_feature_names() -> list[str]:
    return ["modbus_func_dangerous", "modbus_register_address", "modbus_register_sweep_count", "modbus_is_write"]


def modbus_features(flow: dict[str, Any]) -> dict[str, float]:
    """`register_sweep_count` (distinct registers touched by this source
    in its recent request history) is NOT computable from a single flow
    record alone -- it requires cross-request state. The caller (see
    pcap_analysis.py's modbus loop) is responsible for populating
    flow['register_sweep_count'] from a real sliding window BEFORE
    calling this function, the same way ENG-01/02/06 track cross-flow
    state in Redis. If absent, this defaults to 1.0 (a single, first-seen
    register) rather than silently failing -- but that default should
    only ever appear for genuinely isolated calls (e.g. unit tests), not
    ...
    return {
        ...
        "modbus_register_sweep_count": float(flow.get("register_sweep_count", 1)),
